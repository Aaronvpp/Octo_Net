...
class PolarH10:
    ...
    # read, write, indicate – Request stream settings?
    PMD_CHAR1_UUID = "fb005c81-02e7-f387-1cad-8acd2d8df0c8"
    # notify – Start the notify stream?
    PMD_CHAR2_UUID = "fb005c82-02e7-f387-1cad-8acd2d8df0c8"

    ECG_CHAR_UUID = 'FB005C84-5979-11E9-8647-D663BD873D93'
    ...
    ACC_WRITE = bytearray([0x02, 0x02, 0x00, 0x01, 0xC8,
                          0x00, 0x01, 0x01, 0x10, 0x00, 0x02, 0x01, 0x08, 0x00])
    ...
    def __init__(self, bleak_device):
        ...
        self.hr_stream_values = []
        self.hr_stream_times = []
    ...
    def hr_data_conv(self, sender, data):
        ...
        if uint8_format:
            ...
            first_rr_byte += 1
        self.hr_stream_values.append(hr)
        self.hr_stream_times.append(time.time_ns()/1.0e9)
    ...
    def acc_data_conv(self, sender, data):
        ...
        if data[0] == 0x02:
            ...
            timestamp = PolarH10.convert_to_unsigned_long(
                data, 1, 8)/1.0e9  # timestamp of the last sample
            frame_type = data[9]
    ...
    def ecg_data_conv(self, sender, data):
        ...
        if data[0] == 0x00:
            ...
            sample_timestamp = timestamp - (n_samples-1)*time_step
            while offset < len(samples):
                ecg = PolarH10.convert_array_to_signed_int(
                    samples, offset, step)
                offset += step
    ...
    def get_acc_data(self):
        ...
        acc_times = [float(t) for t in self.acc_stream_times]
        acc_times = acc_times - acc_times[0]  # rel to start of acc session
        self.acc_data = {'times': list(acc_times),
                         'values': list(self.acc_stream_values)}

        return self.acc_data

    def get_ibi_data(self):

        ibi_times = [float(t) for t in self.ibi_stream_times]
        # rel to start of acc session time in unix s
        ibi_times = ibi_times - self.acc_stream_start_time
        self.ibi_data = {'times': list(ibi_times),
                         'values': list(self.ibi_stream_values)}

        return self.ibi_data

    def get_hr_data(self):
        hr_times = [float(t) for t in self.hr_stream_times]
        # hr_times = hr_times - self.acc_stream_start_time
        ...
        self.hr_data = {
            "times": list(hr_times),
            "values": list(self.hr_stream_values)
        }
        return self.hr_data

    def get_ecg_data(self):
        ecg_times = [float(t) for t in self.ecg_stream_times]
        # ecg_times = ecg_times - self.acc_stream_start_time
        
        # if self.ecg_data is not None:
        #     self.ecg_data["values"] = []
            
        self.ecg_data = {
            "times": list(ecg_times),
            "values": list(self.ecg_stream_values)
        }
